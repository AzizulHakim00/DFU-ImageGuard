...
def _prepare_target(backup_root: str | Path, run_id: str) -> Path:
    ...
    if current.get("layout_version") != BACKUP_LAYOUT_VERSION:
        # Version 1 copied the entire run after every epoch and could exhaust Drive.
        # Removing it once releases the partial legacy mirror before rebuilding compactly.
        if target.exists():
        ...
        _atomic_json(
            layout,
            {
                ...
                "policy": {
                    "primary_drive": "all checkpoints and artifacts",
                    "secondary_epoch": "active last_resume.pt, best_model.pt and history",
                    "secondary_completed": "portable FP16 checkpoints and non-checkpoint artifacts",
                    "failure_policy": "record DEGRADED and continue; never destroy primary artifacts",
                },
            },
        )
    target.mkdir(parents=True, exist_ok=True)
...
def _write_status(run: Path, target: Path, phase: str, copied: list[dict[str, Any]], skipped: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    try:
        ...
        # The primary status is authoritative even when the secondary volume is full.
        pass
    return status


def _copy_many(run: Path, target: Path, files: Iterable[Path]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def backup_epoch_state(run: str | Path, backup_root: str | Path, run_id: str, trial: str | Path) -> dict[str, Any]:
    run = Path(run)
    trial = Path(trial)
    target = _prepare_target(backup_root, run_id)
    _remove_partial_files(target)

    # Keep only one rolling full-checkpoint secondary copy: the currently active trial.
    ...
    for src in candidates:
        ...
        try:
            meta = _copy_verified(src, dst)
            copied.append({"path": f"_active_trial/{src.name}", "source": _relative_to_run(src, run).as_posix(), **meta})
        except Exception as exc:
            dst.with_name(dst.name + ".partial").unlink(missing_ok=True)
            skipped.append(
                {
    ...
    _atomic_json(active_root / "ACTIVE_TRIAL.json", {"trial": _relative_to_run(trial, run).as_posix(), "updated_at_ns": time.time_ns()})
...
def _completed_artifact_files(run: Path) -> list[Path]:
    ...
    for path in sorted(run.rglob("*")):
        ...
        if "cache" in rel.parts or path.name.endswith(".tmp") or path.name == _STATUS_FILE:
            continue
        if path.suffix == ".pt":
            if path.name == "best_model_portable_fp16.pt" and (path.parent / "COMPLETE.json").exists():
...
def backup_completed_artifacts(run: str | Path, backup_root: str | Path, run_id: str) -> dict[str, Any]:
    run = Path(run)
    target = _prepare_target(backup_root, run_id)
    _remove_partial_files(target)

    # A completed-trial sync no longer needs the rolling full active checkpoint.
    active_root = target / "_active_trial"
    if active_root.exists():
        shutil.rmtree(active_root, ignore_errors=True)
    ...
    return _write_status(run, target, "completed_trial_or_final", copied, skipped)
