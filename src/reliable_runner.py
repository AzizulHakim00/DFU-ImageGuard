...
def _manifest(root: Path):
    return {str(p.relative_to(root)):{"bytes":p.stat().st_size,"sha256":sha256_file(p)} for p in sorted(root.rglob("*")) if p.is_file()}
...
def _infer(model, loader, device):
    import torch
    logits=[]; labels=[]; indices=[]; model.eval()
    with torch.inference_mode():
        for xb,yb,idx in loader:
            logits.append(model(xb.to(device,non_blocking=True)).reshape(-1).float().cpu().numpy()); labels.append(yb.numpy()); indices.append(idx.numpy())
...
def _train_trial(train_df,selection_df,cal_df,test_df,model_key,seed,fold,cfg,settings,trial,backup_callback):
    ...
    optimizer=torch.optim.AdamW(model.parameters(),lr=1e-4,weight_decay=cfg.WEIGHT_DECAY); scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=settings.max_epochs); scaler=torch.amp.GradScaler("cuda",enabled=cfg.USE_AMP)
    train_tf,eval_tf=build_transforms(cfg); tr=build_loader(train_df,train_tf,cfg,True,seed); va=build_loader(selection_df,eval_tf,cfg,False,seed+1)
...
def run_reliable_framework(settings: ReliableSettings|None=None):
    settings=settings or ReliableSettings(); started=time.time(); run=Path(settings.drive_root)/"runs"/settings.run_id; run.mkdir(parents=True,exist_ok=True); marker=run/"DRIVE_SENTINEL.txt"; marker.write_text(str(time.time_ns())); assert marker.read_text(); backup=Path(settings.backup_root)/"runs"/settings.run_id; backup.mkdir(parents=True,exist_ok=True)
    ...
    data=make_outer_folds(assign_duplicate_groups(build_manifest(download_dataset(cfg,dirs),cfg,dirs),cfg,dirs),cfg,dirs); rows=[]; preds=[]
    for fold in settings.folds:
        ...
        for seed in settings.seeds:
            for model_key in settings.models:
                ...
                m,p=_train_trial(train,selection,cal,test,model_key,seed,fold,cfg,settings,trial,lambda:_mirror(run,settings)); rows.append(m); preds.append(p); _mirror(run,settings); _csv(run/"tables"/"fold_seed_metrics.csv",pd.DataFrame(rows)); _csv(run/"tables"/"all_oof_predictions.csv",pd.concat(preds,ignore_index=True))
    ...
    _json(run/"ARTIFACT_MANIFEST.json",_manifest(run)); _mirror(run,settings); final={"run_id":settings.run_id,"completed_trials":len(metrics),"expected_trials":len(settings.folds)*len(settings.seeds)*len(settings.models),"primary_drive":str(run),"secondary_drive":str(backup),"reports":reports,"elapsed_minutes":(time.time()-started)/60}; _json(run/"FINAL_VERIFICATION.json",final); print(json.dumps(final,indent=2)); return final
