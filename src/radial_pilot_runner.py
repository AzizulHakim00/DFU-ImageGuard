...
from .config_data import Config, assign_duplicate_groups, build_manifest, download_dataset, make_inner_partition, make_outer_folds, seed_everything
...
class PilotSettings:
    ...
    github_export_required: bool=False
    ...
    github_auth_source: str='unknown'
...
def _preflight(settings: PilotSettings)->Path:
    ...
    if not any(p.exists() and _under(root,p) for p in valid):
        raise RuntimeError(f'Persistent Drive path required: {root}')
    run=root/'runs'/settings.run_id
    run.mkdir(parents=True,exist_ok=True)
    nonce=hashlib.sha256(f'{time.time_ns()}'.encode()).hexdigest()
    marker=run/'DRIVE_SENTINEL.json'
    atomic_json(marker,{'nonce':nonce,'run':str(run)})
    if json.loads(marker.read_text())['nonce']!=nonce:
    ...
    if settings.require_secondary_drive_backup or settings.secondary_backup_required:
        second=Path(settings.secondary_drive_root)/'runs'/settings.run_id
        second.mkdir(parents=True,exist_ok=True)
        marker2=second/'SECONDARY_SENTINEL.json'
        atomic_json(marker2,{'nonce':nonce})
        if json.loads(marker2.read_text())['nonce']!=nonce:
            raise RuntimeError('Secondary Drive verification failed')
    auth_status={
        'github_export_enabled':bool(settings.github_export),
        'github_export_required':bool(settings.github_export_required),
        'auth_source':settings.github_auth_source,
        'status':'READY' if settings.github_export else 'PENDING_AUTH',
        'training_blocked':False,
    }
    atomic_json(run/'GITHUB_AUTH_STATUS.json',auth_status)
    print('PERSISTENCE PREFLIGHT: PASS',run)
    if not settings.github_export:
        print('GitHub backup: PENDING AUTH; primary and secondary Drive backups remain mandatory and active.')
    return run
...
def _manifest(root: Path)->dict[str,dict[str,Any]]:
    ...
    for p in sorted(root.rglob('*')):
        if p.is_file() and '.git' not in p.parts:
            ...
            out[str(p.relative_to(root))]={'bytes':p.stat().st_size,'sha256':h}
    return out
...
def mirror_secondary(run: Path, settings: PilotSettings)->dict[str,Any]:
    ...
    primary=_manifest(run)
    secondary=_manifest(target)
    if primary!=secondary:
        raise RuntimeError('Secondary Drive SHA-256 verification failed')
    status={'verified':True,'target':str(target),'files':len(primary)}
    atomic_json(run/'SECONDARY_BACKUP_STATUS.json',status)
    return status
...
def export_github(run: Path, settings: PilotSettings, repository_dir: Path)->dict[str,Any]:
    ...
    work=Path('/content/DFU-radial-results-export')
    if work.exists():
        shutil.rmtree(work)
    ...
    if exists:
        ...
        _git(['git','-C',str(work),'checkout','-b',settings.github_branch])
    ...
    for src in run.rglob('*'):
        ...
        if src.suffix=='.pt':
            ...
            if 'portable_fp16' not in src.name:
                continue
        dst=target/rel
        dst.parent.mkdir(parents=True,exist_ok=True)
        shutil.copy2(src,dst)
    for rel in ['src/radial_adapter_model.py','src/radial_pilot_core.py','src/radial_pilot_runner.py','src/checkpoint_backup.py','src/colab_launcher.py']:
        src=repository_dir/rel
        if src.exists():
            dst=target/'source_snapshot'/rel
            dst.parent.mkdir(parents=True,exist_ok=True)
            shutil.copy2(src,dst)
    ...
    if diff.returncode==1:
        _git(['git','-C',str(work),'commit','-m',f'Backup radial pilot {settings.run_id}'])

    if token:
        raw=base64.b64encode(f'x-access-token:{token}'.encode()).decode()
        push_args=['git','-C',str(work),'-c',f'http.extraHeader=Authorization: Basic {raw}','push','origin',f'HEAD:{settings.github_branch}']
    else:
        push_args=['git','-C',str(work),'push','origin',f'HEAD:{settings.github_branch}']
    push=_git(push_args,check=False)
    if push.returncode!=0:
        raise RuntimeError('GitHub backup failed: '+(push.stderr or push.stdout)[-500:])
    sha=_git(['git','-C',str(work),'rev-parse','HEAD']).stdout.strip()
    status={'success':True,'pending':False,'branch':settings.github_branch,'commit':sha,'target':str(target.relative_to(work)),'auth_source':settings.github_auth_source}
    atomic_json(run/'GITHUB_BACKUP_STATUS.json',status)
    return status


def _pending_github_status(run: Path, settings: PilotSettings, reason: str)->dict[str,Any]:
    status={
        'success':False,
        'pending':True,
        ...
        'primary_drive_safe':True,
        'secondary_drive_safe':True,
    }
    atomic_json(run/'GITHUB_BACKUP_STATUS.json',status)
    return status


def _try_export(run: Path, settings: PilotSettings, repository_dir: Path)->dict[str,Any]:
    if not settings.github_export:
        return _pending_github_status(run,settings,'No usable GitHub push authentication was detected before training.')
    try:
        return export_github(run,settings,repository_dir)
    except Exception as exc:
        status=_pending_github_status(run,settings,f'{type(exc).__name__}: {exc}')
        print('GitHub export is pending; Drive backups remain verified:',status['reason'])
        if settings.github_export_required:
            raise
        settings.github_export=False
        return status
...
def run_radial_adapter_pilot(settings: PilotSettings|None=None, repository_dir: str|Path='/content/DFU-ImageGuard-radial'):
    ...
    root=download_dataset(cfg,dirs)
    data=make_outer_folds(assign_duplicate_groups(build_manifest(root,cfg,dirs),cfg,dirs),cfg,dirs)
    train_outer=data[data.outer_fold!=settings.outer_fold].copy()
    test=data[data.outer_fold==settings.outer_fold].copy().reset_index(drop=True)
    inner=make_inner_partition(train_outer,cfg,settings.outer_fold)
    train=inner[inner.inner_role=='train']
    select=inner[inner.inner_role=='selection']
    cal=inner[inner.inner_role=='calibration']
    ...
    for seed in settings.seeds:
        for kind in settings.model_kinds:
            ...
            if settings.require_secondary_drive_backup or settings.secondary_backup_required:
                mirror_secondary(run,settings)
            if settings.github_export_after_each_trial:
                _try_export(run,settings,repo)
    metrics=pd.DataFrame(metric_rows)
    all_pred=pd.concat(predictions,ignore_index=True)
    deltas,verdict=_verdict(metrics)
    atomic_csv(metrics,run/'tables'/'pilot_metrics.csv')
    atomic_csv(all_pred,run/'tables'/'all_test_predictions.csv')
    atomic_csv(deltas,run/'tables'/'paired_seed_deltas.csv')
    atomic_json(run/'PILOT_VERDICT.json',verdict)
    pkl=run/'radial_adapter_pilot_reproducibility.pkl'
    with pkl.open('wb') as f:
        pickle.dump({'settings':asdict(settings),'metrics':metrics.to_dict('records'),'predictions':all_pred.to_dict('records'),'verdict':verdict},f,pickle.HIGHEST_PROTOCOL)
    ...
    github=_try_export(run,settings,repo)
    ...
    if settings.require_secondary_drive_backup or settings.secondary_backup_required:
        mirror_secondary(run,settings)
    if settings.github_export:
        github=_try_export(run,settings,repo)
        final['github_backup']=github
        atomic_json(run/'FINAL_PILOT_VERIFICATION.json',final)
...
def regenerate_from_saved(run_root: str|Path):
    run=Path(run_root)
    metrics=pd.read_csv(run/'tables'/'pilot_metrics.csv')
    deltas,verdict=_verdict(metrics)
    atomic_csv(deltas,run/'tables'/'paired_seed_deltas.csv')
    atomic_json(run/'PILOT_VERDICT.json',verdict)
